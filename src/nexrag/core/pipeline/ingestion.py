...
class IngestionPipeline:
    ...
    def ingest_batch(
        ...
        """
        Ingest many sources with a single batched embedding call.

        Unlike calling ingest() per source, this loads and chunks every source,
        then embeds *all* chunks across the whole batch in one embed() call — the
        expensive part. Embedding N documents becomes 1 provider round-trip
        (subject to the adapter's own batch_size) instead of N. Fingerprint is
        checked once; idempotency and writes still run per source so each source's
        on_conflict decision and per-source result counts are correct.

        Args:
            sources:    Data items to ingest (bytes, str, tuples, etc.).
            loader:     Optional loader override applied to every item.
            metadata:   Optional metadata merged into every Document for every item.
            collection: Override the default collection for the whole batch.

        Returns:
            One IngestionResult per source, in input order. Every result shares the
            batch pipeline_id and reports the batch latency (embedding was shared,
            so per-source latency is not well-defined).

        Raises:
            PipelineError: If no loader is configured, or any stage fails (fail-fast).
        """
        if not sources:
            return []
        ...
        if active_loader is None:
            raise PipelineError(
                "No loader configured. Pass a loader to ingest_batch() or set one "
                "at pipeline construction. Use ingest_documents() to skip the loader stage.",
                stage="loader",
                component="ingestion",
                pipeline_id=pipeline_id,
            )
        ...
        try:
            # 1. Load every source into its own document group.
            ...
            # 2. Chunk each source, tracking which chunk slice belongs to which source.
            ...
            # 3. Embed ALL chunks across the batch in a single call — the batching win.
            all_embeddings = self._run_embedder(all_chunks, pipeline_id)

            # 4. Fingerprint check once for the target collection.
            self._run_fingerprint_check(active_collection, pipeline_id)

            # 5. Idempotency + write per source (preserves per-source semantics/results).
            ...
            if isinstance(e, PipelineError):
                raise
            raise PipelineError(
                f"Unexpected error during batch ingestion: {e}",
                stage="pipeline",
                component="ingestion",
                pipeline_id=pipeline_id,
                cause=e,
