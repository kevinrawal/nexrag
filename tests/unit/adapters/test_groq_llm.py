...
class TestGroqLLM:
    ...
    def test_rate_limit_error_raises_llm_rate_limit_error(self):
        ...
        llm = _make_llm()
        llm._client.chat.completions.create.side_effect = groq.RateLimitError(
    ...
    def test_timeout_error_raises_llm_timeout_error(self):
        ...
        llm = _make_llm()
        llm._client.chat.completions.create.side_effect = groq.APITimeoutError(
            request=MagicMock()
        )
    ...
    def test_auth_error_raises_llm_error(self):
        ...
        llm = _make_llm()
        llm._client.chat.completions.create.side_effect = groq.AuthenticationError(
    ...
    def test_async_stream_yields_tokens_and_skips_choiceless_chunks(self):
        ...
        with (
            ...
            patch("groq.AsyncGroq", return_value=mock_async_client),
        ):
            llm = GroqLLM(api_key="gsk-test")
    ...
    def test_build_messages_with_separator(self):
        ...
        msgs = GroqLLM._build_messages("sys\n\n---\n\nuser")
    ...
    def test_build_messages_without_separator(self):
        ...
        msgs = GroqLLM._build_messages("just user")
    ...
    def test_retries_on_rate_limit_then_succeeds(self):
        ...
        rate_err = groq.RateLimitError("rate limited", response=MagicMock(), body={})
    ...
    def test_auth_error_not_retried(self):
        ...
        llm = _make_llm()
        auth_err = groq.AuthenticationError("bad key", response=MagicMock(), body={})
    ...
    def test_max_retries_zero_never_retries(self):
        ...
        with patch("groq.Groq", return_value=mock_client):
            llm = GroqLLM(api_key="gsk-test", max_retries=0)
        llm._client = mock_client
